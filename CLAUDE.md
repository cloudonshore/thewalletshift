...
## Repo layout
- `web/` — Next.js 15 app. App Router, TS, Tailwind, Recharts. **Three pages:** `/` (dashboard), `/services` (**browsable directory of the 711 real-service agents** — search + category/protocol filters, each row opens a detail modal with endpoints + live A2A skills/MCP tools; reads bundled `web/src/data/services.json` built by `scripts/build-services-directory.mjs`), and **`/collectibles`** (**gallery of the 1,268 collectible-tier agents grouped into the NFT *collections* they are** — FREAK 251 / Normie 1,002 / experimental tail 15; each a card with shared skill set + trait breakdown (FREAK factions / Normie personas), opening a modal with a searchable member browser; reads bundled `web/src/data/collectibles.json` built by `scripts/build-collectibles.mjs`). Nav = Home + Services + Collectibles. **Note: FREAK/Normie are NOT separate NFT contracts — they're batches of tokens minted into the one ERC-8004 Identity registry ERC-721, interleaved in one shared token-id space; the "collection" grouping is off-chain convention (shared serving host + naming + traits), not an on-chain field. FREAK is distributed (251 distinct holders); Normie is still 100% held by one deployer wallet `0xde152afb…` (minted, undistributed).** *(Removed 2026-06-13: `/agents` virtualized table + its `/api/agents` route + `lib/gcs.ts` + `agents-table.tsx`; the `/cards` page + `card-charts.tsx` + `lib/cards.ts`; and the older `/explore` page. `agents.json` stays in GCS as a pipeline source.)* *(Removed 2026-06-14 in submission cleanup: orphaned `components/charts.tsx` and the committed `cards.json` — both dead after the `/cards`/`/agents` removals.)*
- `sql/` — validated BigQuery query library (`erc8004_queries.sql`).
- `scripts/` — BQ→JSON exports: `export-metrics.sh` (dashboard), `export-agents.sh` (full table → GCS); **off-chain card pipeline** `export-offchain-uris.sh` (BQ worklist) → `fetch-cards.mjs` (Node HTTP fetch+parse; BQ can't egress) → `merge-cards.mjs` (fold off-chain card fields into `agents.json`); **classification pipeline** (see below) `export-onchain-callable.sh` + `fetch-cards.mjs` → `build-enrich-input.mjs` → `fetch-skills.mjs` (2nd-hop A2A skills/MCP tools) → `build-corpus.mjs` → `prep-discovery.mjs` (stratified sample → `_disc/`) → taxonomy-discovery **Workflow** → `prep-classify.mjs` (chunk corpus → `_cls/`) → classify-agents **Workflow** → `build-classified.mjs`; **endpoint health probe** `probe-health.mjs` (reads the committed `services.json`, probes each service-tier endpoint → gitignored `health.json`; re-run `build-services-directory.mjs` to fold per-endpoint `health` back in. Run order: `build-services-directory.mjs` → `probe-health.mjs` → `build-services-directory.mjs` again the first time; thereafter probe → build. **The probe makes minimal *real* calls to third-party x402 endpoints** — but only to `x402:true` a2a services, one safe-named (allowlisted, non-destructive) skill per service, and the `402` returns before any charge or work); plus `explore.sh '<SQL>'` ad-hoc runner.
...
- **Exploration tools:** `scripts/explore.sh '<SQL>'` (ad-hoc, `\`T\`` = the table, prints bytes/cache; `-n` = dry-run cost) · `export-agents.sh` → `agents.json` (full table, current owner) → GCS.
...
- **ENS in the card is self-declared, but we verify it live** (BUILT 2026-06-14): `web/src/lib/ens.ts` + `GET /api/ens?name=&address=` resolve the claimed name on Ethereum mainnet (viem, no baked values) and grade it against the agent's on-chain owner — verified / mismatch / unconfigured, plus reverse primary-name + ENSIP-26 text records. Owner address is carried into `services.json`; shown as a live badge in the `/services` modal and linked from `/api/services/{id}` (`ens_verify`) and `/SKILL.md`. `kind` (onchain/https/ipfs/empty) is still **derived by us** from the URI prefix, not an on-chain field.
- **Card indexing (BUILT):** on-chain cards (9,520) decode in SQL for **free** (per-agent fields baked into `agents.json` by `export-agents.sh`); off-chain `https`/`ipfs` (4,662) fetched by `fetch-cards.mjs` (BQ **cannot egress** — Node fetches, runs locally today, promote to Cloud Run Job for cron). ~2,012/4,662 links return a live card (43%). ⚠️ **`ag0.xyz` = 1,985 off-chain agents all point at the bare homepage** (placeholder spam, 43% of off-chain). 52% of all agents have no card.
