...
## Repo layout
- `web/` — Next.js 15 app. App Router, TS, Tailwind, Recharts, TanStack Table+Virtual. Pages: `/` (dashboard), `/agents` (virtualized browser of all 34.5k), `/services` (**LLM-classified analytics of the callable agents — what they actually do**), `/cards` (Agent Card analytics + interactability). Routes: `/api/agents` (serves full set from GCS via `web/src/lib/gcs.ts`). *(`/explore` raw-event page removed 2026-06-13 — a searchable agent list will live under `/services` instead.)*
