...
#
# Uses the BigQuery REST API over curl (forced IPv4). The `bq` CLI uses Python
# httplib2 which stalls 60s when the host resolves to an unreachable IPv6 address
# (common behind some VPNs); curl -4 sidesteps that. Queries hit the materialized
# table thewalletshift.erc8004.logs_2026 (free). This is the basis for the
# scheduled production rollup (which runs inside GCP and has no IPv6 issue).
set -euo pipefail
...
DAY="${1:?usage: export-metrics.sh YYYY-MM-DD}"
TMP="$(mktemp -d)"
TOKEN="$(gcloud auth print-access-token)"
URL="https://bigquery.googleapis.com/bigquery/v2/projects/$PROJECT/queries"

# run_query <sql> -> clean JSON array of row objects on stdout
run_query() {
...
run_query "SELECT FORMAT_DATE('%Y-%m-%d', DATE(block_timestamp)) AS day, COUNT(*) AS new_agents
FROM \`$TBL\` WHERE address='$ID' AND topics[SAFE_OFFSET(0)]='$REG'
GROUP BY day ORDER BY day" > "$TMP/growth.json"

echo "3/6 top owners..."
run_query "SELECT CONCAT('0x',SUBSTR(topics[SAFE_OFFSET(2)],27)) AS owner,
...
run_query "SELECT CASE
   ...
   WHEN STARTS_WITH(uri,'http:') THEN 'http'
